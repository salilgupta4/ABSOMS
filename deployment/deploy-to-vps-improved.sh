...
# ABS OMS Automated VPS Deployment Script - Improved Version
...
echo -e "${BLUE}🚀 ABS OMS Automated VPS Deployment - Improved${NC}"
echo -e "${BLUE}================================================${NC}"
...
echo "Please provide your VPS SSH details:"
read -p "SSH Username (try: root, admin, ubuntu, or your HestiaCP username): " SSH_USER
read -p "SSH Password: " -s VPS_PASSWORD
echo
...
# Function to run commands on VPS with improved error handling
run_remote() {
    if command -v sshpass >/dev/null 2>&1; then
        sshpass -p "$VPS_PASSWORD" ssh -p "$SSH_PORT" -o StrictHostKeyChecking=no -o ConnectTimeout=30 "$SSH_USER@$VPS_IP" "$1"
    else
        log_warning "sshpass not found. Install with: brew install hudochenkov/sshpass/sshpass"
        log_info "Please run this command manually on your VPS:"
        echo "$1"
        read -p "Press Enter after running the command on your VPS..."
    fi
}
...
copy_to_vps() {
    if command -v sshpass >/dev/null 2>&1; then
        sshpass -p "$VPS_PASSWORD" scp -P "$SSH_PORT" -r "$1" "$SSH_USER@$VPS_IP:$2"
    else
        log_warning "Please manually copy $1 to $2 on your VPS"
        read -p "Press Enter after copying..."
    fi
}

# Test SSH connection with multiple methods
log_info "Testing SSH connection..."

# Method 1: Try with sshpass
if command -v sshpass >/dev/null 2>&1; then
    if sshpass -p "$VPS_PASSWORD" ssh -p "$SSH_PORT" -o StrictHostKeyChecking=no -o ConnectTimeout=30 "$SSH_USER@$VPS_IP" "echo 'SSH connection successful'"; then
        log_success "SSH connection established with sshpass"
        USE_SSHPASS=true
    else
        log_warning "SSH connection with sshpass failed"
        USE_SSHPASS=false
    fi
else
    log_warning "sshpass not installed. Install with: brew install hudochenkov/sshpass/sshpass"
    USE_SSHPASS=false
fi

# Method 2: Manual connection test
if [ "$USE_SSHPASS" = false ]; then
    log_info "Please test SSH connection manually:"
    echo "ssh -p $SSH_PORT $SSH_USER@$VPS_IP"
    read -p "Can you connect? (y/n): " can_connect
    if [ "$can_connect" != "y" ]; then
        log_error "Cannot establish SSH connection. Please check:"
        echo "1. Username: Try root, admin, ubuntu, or your HestiaCP username"
        echo "2. Password: Verify in your Hostinger/HestiaCP panel"
        echo "3. Port: Default is 22, but might be different"
        echo "4. SSH Settings: Root login might be disabled"
        echo ""
        echo "Common Hostinger/HestiaCP usernames:"
        echo "- root (if root login enabled)"
        echo "- admin (HestiaCP admin user)"
        echo "- Your domain username"
        echo ""
        echo "Check your Hostinger panel for SSH credentials"
        exit 1
    fi
fi
...
npm run db:init << DB_INIT || echo "Database initialization may need manual setup"
Admin User
admin@$DOMAIN
SecureAdmin123
DB_INIT
...
certbot --nginx --non-interactive --agree-tos --email $SSL_EMAIL -d $DOMAIN -d www.$DOMAIN || echo "SSL installation failed - may need manual setup"
...
mysqldump -u __DB_USER__ -p'__DB_PASSWORD__' __DB_NAME__ > /backups/abs_oms_$DATE.sql
...
sed -i "s/__DB_USER__/$DB_USER/g" /opt/backup-db.sh
sed -i "s/__DB_PASSWORD__/$DB_PASSWORD/g" /opt/backup-db.sh
sed -i "s/__DB_NAME__/$DB_NAME/g" /opt/backup-db.sh
...
# Replace variables in update script
sed -i "s|__APP_DIR__|$APP_DIR|g" /opt/update-app.sh
...
if [ "$USE_SSHPASS" = true ]; then
    ...
    sshpass -p "$VPS_PASSWORD" scp -P "$SSH_PORT" /tmp/vps_setup.sh "$SSH_USER@$VPS_IP:/tmp/"
    
    # Execute deployment script on VPS
    log_info "Executing deployment on VPS..."
    sshpass -p "$VPS_PASSWORD" ssh -p "$SSH_PORT" "$SSH_USER@$VPS_IP" "chmod +x /tmp/vps_setup.sh && sudo /tmp/vps_setup.sh"
else
    log_info "Manual deployment mode..."
    log_warning "Please copy /tmp/vps_setup.sh to your VPS and run it as root:"
    echo "scp -P $SSH_PORT /tmp/vps_setup.sh $SSH_USER@$VPS_IP:/tmp/"
    echo "ssh -p $SSH_PORT $SSH_USER@$VPS_IP"
    echo "sudo chmod +x /tmp/vps_setup.sh"
    echo "sudo /tmp/vps_setup.sh"
    read -p "Press Enter after running the deployment script..."
fi
...
echo -e "ssh -p $SSH_PORT $SSH_USER@$VPS_IP"
